...
public class DesignDisplayPage extends DesignDisplayPageGen<DesignDisplayGenPage> {
	...
	protected void _schoolEnrollment(Wrap<SiteEnrollment> c) {
	...
	protected void _enrollments(Wrap<List<SiteEnrollment>> c) {
		...
		SiteEnrollment enrollment = null;
		List<SiteEnrollment> enrollmentEnrollments = null;
	...
	protected void _enrollmentBlocks(List<SiteEnrollment> c) {
	}

	protected void _enrollmentGroups(Wrap<List<SiteEnrollment>> c) {
	}

	protected void _enrollmentBlock(Wrap<SiteEnrollment> c) {
	}

	protected void _enrollmentGroup(Wrap<SiteEnrollment> c) {
	}

	protected void _enrollmentEnrollment(Wrap<SiteEnrollment> c) {
	}
	...
	protected void _yearSearch(SearchList<SchoolYear> l) {
		...
		Long yearKey = Optional.ofNullable(enrollmentSearch.first()).map(SiteEnrollment::getYearKey).orElse(null);
	...
	protected void _schoolSearch(SearchList<School> l) {
		...
		Long schoolKey = Optional.ofNullable(enrollmentSearch.first()).map(SiteEnrollment::getSchoolKey).orElse(null);
	...
	protected void _blocks(Wrap<List<SchoolBlock>> c) {
		...
		SiteEnrollment schoolEnrollment = enrollmentSearch.size() == 1 ? enrollmentSearch.first() : null;
...
pare(ageStartCurrent, ageStartBefore) != 0) {
