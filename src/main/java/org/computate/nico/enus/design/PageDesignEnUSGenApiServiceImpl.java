...
import org.computate.nico.enus.design.PageDesignEnUSGenApiServiceImpl;
import org.computate.nico.enus.design.PageDesign;
import org.computate.nico.enus.html.part.HtmlPartEnUSGenApiServiceImpl;
			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
					...
					case "htmlPartKeys":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entityVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							if(l != null) {
								SearchList<org.computate.nico.enus.html.part.HtmlPart> searchList = new SearchList<org.computate.nico.enus.html.part.HtmlPart>();
								searchList.setQuery("*:*");
								searchList.setStore(true);
								searchList.setC(org.computate.nico.enus.html.part.HtmlPart.class);
								searchList.addFilterQuery("deleted_indexed_boolean:false");
								searchList.addFilterQuery("archived_indexed_boolean:false");
								searchList.addFilterQuery((inheritPk ? "inheritPk" : "pk") + "_indexed_long:" + l);
								searchList.initDeepSearchList(siteRequest);
