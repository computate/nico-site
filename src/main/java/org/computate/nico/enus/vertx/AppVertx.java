...
import org.computate.nico.enus.cluster.ClusterEnUSGenApiService;
import org.computate.nico.enus.config.SiteConfig;
import org.computate.nico.enus.context.SiteContextEnUS;
import org.computate.nico.enus.design.PageDesignEnUSGenApiService;
import org.computate.nico.enus.enrollment.SiteEnrollmentEnUSGenApiService;
import org.computate.nico.enus.html.part.HtmlPartEnUSGenApiService;
...
public class AppVertx extends AppVertxGen<AbstractVerticle> {
	...
	private Promise<Void> configureOpenApi() {
		...
		//		ClusteredSessionStore sessionStore = ClusteredSessionStore.create(vertx);
				LocalSessionStore sessionStore = LocalSessionStore.create(vertx, "nico-site-sessions");
...
//						routerFactory.addSecurityHandler("openIdConnect", authHandler);
	...
	private Promise<Void> startServer() {
		...
		SiteEnrollmentEnUSGenApiService.registerService(siteContextEnUS, vertx);

		Router siteRouter = siteContextEnUS.getRouter();

		StaticHandler staticHandler = StaticHandler.create().setCachingEnabled(false).setFilesReadOnly(true);
		if("nico-dev.computate.org".equals(siteConfig.getSiteHostName())) {
			staticHandler.setAllowRootFileSystemAccess(true);
			staticHandler.setWebRoot("/usr/local/src/nico-site-static");
		}
